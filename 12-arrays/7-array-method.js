// .method( ?input)
const students = []; // created an array

// add value to array by using .push(NEW_VALUE) method
students.push('syed');

// add value using lenth property ARRAY[ARRAY.length] = NEW_VALUE
...
const valueTwo = fnWithoutReturn (); // it will return undifined
...
[]. forEach
/*
(method) Array<never>.forEach(callbackfn: (value: never, index: number, array: never[]) => void, thisArg?: any): void
...
/*
(method) Array<never>.map<U>(callbackfn: (value:never,index:number, array: never[]) => U, thisArg?: any): U [] 
calls a defined callback function each element of a Array, and returns an Array that contains the results.*/

const welcome_students_using_forEach = students.forEach((students) => {
...
const welcome_students_using_map = students.map((students) => {
    const message = `Welcome ${students}`;
    console.log(message);
    return message;
});
...
const shoppingPrices = [56,520,650,420]; //apply discount of 10%;
...
['syed', 'hafeez','sameer'].forEach( (value) => {console.log(`welcome {value}`) } );

// 'syed' => ('syed') => {console.log(`welcome syed`)}
// 'hafeez' => ('hafeez') => {console.log(`welcome hafeez`)}
// 'sameer'
